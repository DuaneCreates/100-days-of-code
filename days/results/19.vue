<template>
  <section class="flex flex-col bg-blue-lighter p-8">
    <input
      v-model="q"
      class="bg-yellow-lightest text-yellow-darker p-2 text-2xl"
    ...
    <div class="mt-4">
      <ul v-if="!loading_models">
        <li v-for="m in models" :key="m.Model_Name">{{ m.Model_Name }}</li>
      </ul>
      <p v-if="!loading_models && message" class="m-2">{{ message }}</p>
    </div>
  </section>
</template>
  ...
  methods: {
    throttledMethod: _.debounce(
      function() {
        ...
        axios
          .get(
            `${api}vehicles/GetModelsForMakeYear/make/${
              this.q
          ...
          .then(({ data }) => {
            ...
            this.message =
              this.models.length === 0 ? 'No cars have been found' : null
